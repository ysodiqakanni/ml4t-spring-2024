...
def sharpe_ration_fn(allocations, normPrices):
...
def getStatistics(allocations, normPrices):
    ...
    dailyReturns = compute_daily_returns(dfPortfolioValue)
    cr = (dfPortfolioValue[-1]/dfPortfolioValue[0]) -1
    adr = dailyReturns.mean()
    sddr = dailyReturns.std()
    sr = math.sqrt(252) * adr / sddr
    return [cr, adr, sddr, sr, dfPortfolioValue[-1]]
