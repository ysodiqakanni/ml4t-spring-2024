...
class QLearner(object):  		  	   		 	   			  		 			     			  	 
    ...
    def query(self, s_prime, r):  		  	   		 	   			  		 			     			  	 
        ...
        # s_prime = new state you're in
        # r = reward for taking the last action
        # remember what was the last state and save it
        # what was the last action
        # experience tuple <s,a,s_prime,r> = <current_state, prev_state, actionFromPrevToCurr, rewardForActionA>
        ...
        random_action_probability = np.random.rand()
        if random_action_probability < self.rar:
            # then we choose a random action
            action = rand.randint(0, self.num_actions - 1)
            # now update rar according to the decay rate
            #self.rar *= self.radr
        else:
            # let's find the best action to take from the Q table
